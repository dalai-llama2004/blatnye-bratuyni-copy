...
export const adminService = {
  ...
  async createZone(data: ZoneCreate): Promise<Zone> {
    const response = await api.post('/bookings/admin/zones', data);
    return response.data;
  },

  // Обновить зону
  async updateZone(zoneId: number, data: ZoneUpdate): Promise<Zone> {
    const response = await api.patch(`/bookings/admin/zones/${zoneId}`, data);
    return response.data;
  },

  // Удалить зону
  async deleteZone(zoneId: number): Promise<void> {
    await api.delete(`/bookings/admin/zones/${zoneId}`);
  },
  ...
  async closeZone(
    ...
    const response = await api.post(
      `/bookings/admin/zones/${zoneId}/close`,
      data
    );
    return response.data;
  },
